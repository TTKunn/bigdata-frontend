...
6. [销售统计接口](#销售统计接口)
   ...
   - [6.4 获取过去七天销售额统计](#64-获取过去七天销售额统计)
   - [6.5 获取过去七天订单数统计](#65-获取过去七天订单数统计)
   - [6.6 获取最畅销商品排行榜](#66-获取最畅销商品排行榜)
   - [6.7 SSE实时推送销售统计更新](#67-sse实时推送销售统计更新)
7. [错误码说明](#错误码说明)
8. [测试用例](#测试用例)
    ...
    "products": [
      {
        ...
        "createTime": null,
        "status": "ACTIVE",
        "imageUrl": "http://localhost:8080/api/images/redmibuds6.jpg"
      },
      {
        "id": "000100000003",
        "name": "小米17 Ultra",
        ...
        "createTime": null,
        "status": "ACTIVE",
        "imageUrl": "http://localhost:8080/api/images/xiaomi17ultra.png"
      },
      {
        "id": "000100000002",
        "name": "华为Mate60 Pro",
        ...
        "createTime": null,
        "status": "ACTIVE",
        "imageUrl": "http://localhost:8080/api/images/mate60pro.png"
      },
      {
        "id": "000100000001",
        "name": "美的空调",
        ...
        "createTime": null,
        "status": "ACTIVE",
        "imageUrl": "http://localhost:8080/api/images/kongtiao.png"
      }
    ],
    "hasNext": false,
    "hasPrevious": false,
...
| imageUrl | string | 商品图片HTTP访问URL，可直接在浏览器或img标签中使用 |

**说明：**
- `imageUrl` 字段返回的是可直接访问的HTTP URL，格式为：`http://localhost:8080/api/images/{filename}`
- 前端可以直接使用该URL在 `<img>` 标签中显示图片
- 如果商品没有图片，该字段为 `null`
...
```json
{
  ...
  "data": {
    ...
    "image": {
      ...
      "size": 245760,
      "uploadTime": "2026-01-07T10:30:00Z",
      "url": "http://localhost:8080/api/images/1704625800000_main.jpg"
    },
    "stock": {
      "total": 1000,
      "safe": 100,
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| id | string | HDFS存储路径（内部使用） |
| type | string | 图片类型（main-主图） |
| filename | string | 原始文件名 |
| size | number | 文件大小（字节） |
| uploadTime | string | 上传时间 |
| url | string | HTTP访问URL，可直接在浏览器或img标签中使用 |

**说明：**
- `image.url` 字段返回的是可直接访问的HTTP URL，格式为：`http://localhost:8080/api/images/{filename}`
- 前端可以直接使用该URL在 `<img>` 标签中显示图片
- `image.id` 字段是HDFS内部路径，前端无需关注
...
| items[].imageUrl | string | 商品图片HTTP访问URL，可直接在浏览器或img标签中使用 |
| totalQuantity | integer | 购物车商品总数量 |
| totalAmount | number | 购物车商品总金额 |

**说明：**
- `items[].imageUrl` 字段返回的是可直接访问的HTTP URL，格式为：`http://localhost:8080/api/images/{filename}`
- 前端可以直接使用该URL在 `<img>` 标签中显示图片
- 如果商品没有图片，该字段为 `null`
...
#### 接口描述
查询用户的订单列表，支持按订单状态筛选和分页查询。订单按创建时间倒序排列（最新的订单在前）。

**分页说明：**
- 支持分页查询，默认每页10条记录
- 返回结果包含完整的分页信息（当前页、每页大小、总记录数、总页数）
- 订单总数会根据状态筛选条件动态计算
...
| pagination.page | integer | 当前页码（从1开始） |
| pagination.size | integer | 每页大小 |
| pagination.total | long | 总记录数（根据筛选条件统计的订单总数） |
| pagination.totalPages | integer | 总页数（根据总记录数和每页大小计算） |

**分页信息说明：**
- `total`：表示符合筛选条件的订单总数，不是当前页的记录数
- `totalPages`：总页数 = Math.ceil(total / size)
- 如果指定了 `status` 参数，`total` 只统计该状态的订单数量
- 如果未指定 `status` 参数，`total` 统计所有状态的订单数量
...
#### 接口描述
获取过去七天（包括今天）每天的销售额统计信息，包括每天的销售额、订单数、平均客单价，以及七天的总销售额和总订单数。

#### 请求信息
- **接口路径**：`GET /api/statistics/seven-days-sales`
- **请求方法**：GET
  ...
  "data": {
    "dailySales": [
      ...
      },
      {
        ...
        "sales": 6500.00,
        "orderCount": 2,
        "averageOrderValue": 3250.00
      },
      {
        "date": "20260107",
        "sales": 9200.00,
        "orderCount": 3,
        "averageOrderValue": 3066.67
      },
      {
        "date": "20260108",
        "sales": 15800.00,
        "orderCount": 5,
        "averageOrderValue": 3160.00
      }
    ],
    "totalSales": 57000.00,
    "totalOrders": 19
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| dailySales | array | 每天的销售数据列表（按日期升序，从7天前到今天） |
| dailySales[].date | string | 日期（yyyyMMdd格式） |
| dailySales[].sales | number | 当日销售额（单位：元） |
| dailySales[].orderCount | integer | 当日订单数 |
| dailySales[].averageOrderValue | number | 当日平均客单价（单位：元） |
| totalSales | number | 七天总销售额（单位：元） |
| totalOrders | integer | 七天总订单数 |
...
##### curl命令
```bash
curl -X GET "http://localhost:8080/api/statistics/seven-days-sales"
```

##### ApiFox测试步骤
1. 创建新请求，方法选择GET
2. URL填写：`http://localhost:8080/api/statistics/seven-days-sales`
3. 点击"发送"按钮
4. 验证返回的七天销售数据
...
#### 接口描述
获取过去七天（包括今天）每天的订单数量统计信息，以及七天的总订单数。

#### 请求信息
- **接口路径**：`GET /api/statistics/seven-days-orders`
- **请求方法**：GET
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| dailyOrders | array | 每天的订单数据列表（按日期升序，从7天前到今天） |
| dailyOrders[].date | string | 日期（yyyyMMdd格式） |
| dailyOrders[].orderCount | integer | 当日订单数 |
| totalOrders | integer | 七天总订单数 |
...
##### curl命令
```bash
curl -X GET "http://localhost:8080/api/statistics/seven-days-orders"
```

##### ApiFox测试步骤
1. 创建新请求，方法选择GET
2. URL填写：`http://localhost:8080/api/statistics/seven-days-orders`
3. 点击"发送"按钮
4. 验证返回的七天订单数据
...
#### 接口描述
通过Server-Sent Events (SSE)技术实时推送销售统计数据更新，前端无需轮询或手动刷新即可自动接收最新数据。

#### 请求信息
- **接口路径**：`GET /api/statistics/stream`
- **请求方法**：GET
...
| Header | 值 | 必填 | 说明 |
|--------|---|------|------|
| Accept | text/event-stream | 是 | 指定接收SSE流 |
...
###### 2. statistics-update（统计数据更新事件）
当Redis统计数据更新时推送的事件，包含以下几种类型：

**总销售额更新 (total_sales)**
```json
{
  "eventType": "total_sales",
  "updateTime": "2026-01-08T10:30:00",
  "data": {
    "totalSales": 123456.78,
    "completedOrders": 1234,
    "lastUpdate": 1704700800000
  }
}
```
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| eventType | string | 事件类型：total_sales（总销售额）、daily_sales（当日销售额）、product_sales（商品销量） |
| updateTime | string | 事件发生时间（ISO 8601格式） |
| data | object | 具体的更新数据，根据eventType不同而不同 |

#### 业务说明
- SSE连接超时时间为30分钟，超时后自动断开
- 客户端断开后会自动尝试重连（浏览器EventSource内置机制）
- 统计数据更新时（定时任务每20秒处理一次），自动推送给所有连接的客户端
- 支持多个客户端同时连接
- 连接建立后立即发送`connected`事件确认连接成功
- 后续每次Redis统计数据更新时发送`statistics-update`事件

#### 前端接入示例

##### JavaScript (原生EventSource)
```javascript
// 建立SSE连接
const eventSource = new EventSource('http://localhost:8080/api/statistics/stream');
...
// 监听统计数据更新事件
eventSource.addEventListener('statistics-update', (event) => {
  const updateEvent = JSON.parse(event.data);

  switch(updateEvent.eventType) {
    case 'total_sales':
      // 更新总销售额显示
      updateTotalSalesUI(updateEvent.data);
      break;
    case 'daily_sales':
      // 更新当日销售额显示
      updateDailySalesUI(updateEvent.data);
      break;
    case 'product_sales':
      // 更新商品排行榜显示
      updateProductRankUI(updateEvent.data);
      break;
  }
});
...
<template>
  <div>
    <h2>实时销售统计</h2>
    <div>总销售额: {{ totalSales }}</div>
    <div>已完成订单: {{ completedOrders }}</div>
...
onMounted(() => {
  // 建立SSE连接
  eventSource = new EventSource('http://localhost:8080/api/statistics/stream');

  eventSource.addEventListener('statistics-update', (event) => {
    const updateEvent = JSON.parse(event.data);

    if (updateEvent.eventType === 'total_sales') {
      totalSales.value = updateEvent.data.totalSales;
      completedOrders.value = updateEvent.data.completedOrders;
      lastUpdate.value = new Date(updateEvent.data.lastUpdate).toLocaleString();
    }
  });
});
...
function SalesStatistics() {
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener('statistics-update', (event) => {
      ...
      if (updateEvent.eventType === 'total_sales') {
        setTotalSales(updateEvent.data.totalSales);
        setCompletedOrders(updateEvent.data.completedOrders);
        setLastUpdate(new Date(updateEvent.data.lastUpdate).toLocaleString());
      }
    });
...
##### curl命令（测试连接）
```bash
curl -N -H "Accept: text/event-stream" http://localhost:8080/api/statistics/stream
```
...
2. 在Console中执行以下代码：
```javascript
const es = new EventSource('http://localhost:8080/api/statistics/stream');
es.addEventListener('connected', e => console.log('Connected:', e.data));
es.addEventListener('statistics-update', e => console.log('Update:', e.data));
```
3. 创建并完成一个订单
4. 等待约20秒（定时任务执行周期）
5. 观察Console中是否收到`statistics-update`事件

#### 技术说明
- 使用Redis Pub/Sub机制实现消息分发
- 后端订阅`statistics:update`频道
- 统计数据更新时发布消息到该频道
- SSE服务监听频道并推送给所有连接的客户端
- 支持水平扩展（多实例部署时通过Redis共享消息）

#### 注意事项
1. **跨域问题**：如果前端和后端不在同一域名，需要配置CORS
2. **连接数限制**：浏览器对同一域名的SSE连接数有限制（通常6个）
3. **网络稳定性**：移动网络或不稳定网络可能导致频繁重连
4. **资源占用**：长连接会占用服务器资源，建议监控连接数
5. **数据延迟**：统计数据更新有约20秒延迟（定时任务周期）
